...
INSERT INTO businesses (id, owner_id, name, business_type, phone, address, description, business_hours, status) VALUES
                                                                                                                    (1, 2, '강남 헤어살롱', 'BEAUTY_SHOP', '02-1111-1111', '서울특별시 강남구 테헤란로 123', '강남역 5분거리 프리미엄 헤어살롱입니다.',
                                                                                                                     '{
...
INSERT INTO business_settings (business_id, booking_interval, auto_confirm, allow_online_booking,
                               ...
                               kakao_enabled,
                               payment_methods, require_deposit, deposit_amount,
                               allow_cancellation, cancel_deadline_hours, no_show_penalty_enabled,
                               timezone, language) VALUES
-- 강남 헤어살롱 설정
(1,
 30,           -- booking_interval: 30분 단위
 'N',          -- auto_confirm: 수동 확정
 'Y',          -- allow_online_booking: 온라인 예약 허용
 30,           -- max_advance_booking_days: 30일 전까지 예약 가능
 2,            -- min_advance_booking_hours: 최소 2시간 전 예약
 'Y',          -- send_confirmation_sms: 확정 SMS 발송
 'Y',          -- send_reminder_sms: 알림 SMS 발송
 24,           -- reminder_hours_before: 24시간 전 알림
 'Y',          -- send_cancel_sms: 취소 SMS 발송
 'N',          -- kakao_enabled: 카카오톡 미사용
 'CARD,CASH,TRANSFER',  -- payment_methods
 'N',          -- require_deposit: 예약금 불필요
 0,            -- deposit_amount
 'Y',          -- allow_cancellation: 취소 허용
 24,           -- cancel_deadline_hours: 24시간 전까지 취소 가능
 'N',          -- no_show_penalty_enabled: 노쇼 페널티 미사용
 'Asia/Seoul', -- timezone
 'ko'),        -- language

-- 홍대 필라테스 설정
(2,
 60,           -- booking_interval: 60분 단위
 'Y',          -- auto_confirm: 자동 확정
 'Y',          -- allow_online_booking: 온라인 예약 허용
 14,           -- max_advance_booking_days: 14일 전까지 예약 가능
 12,           -- min_advance_booking_hours: 최소 12시간 전 예약
 'Y',          -- send_confirmation_sms
 'Y',          -- send_reminder_sms
 24,           -- reminder_hours_before: 24시간 전 알림
 'Y',          -- send_cancel_sms
 'N',          -- kakao_enabled: 카카오톡 미사용
 'CARD,TRANSFER',  -- payment_methods: 카드/계좌이체만
 'Y',          -- require_deposit: 예약금 필요
 30000,        -- deposit_amount: 3만원
 'Y',          -- allow_cancellation: 취소 허용
 12,           -- cancel_deadline_hours: 12시간 전까지 취소 가능
 'Y',          -- no_show_penalty_enabled: 노쇼 페널티 사용
 'Asia/Seoul', -- timezone
 'ko');        -- language
...
(1, 1, 1, 1, '2025-12-20', '14:00', '16:30',
 ...
 150, 115000, 'COMPLETED', 'RES-20251220-0001', '볼륨감 많이 살려주세요', '고객님 만족하심'),

(2, 1, 2, 1, '2025-12-28', '10:00', '11:30',
 '[{"id": 5, "name": "전체 염색", "price": 70000, "duration": 90}]'::jsonb,
 90, 70000, 'COMPLETED', 'RES-20251228-0001', '', '애쉬 브라운으로 시술'),

(3, 1, 4, 1, '2026-01-02', '15:00', '16:00',
 '[{"id": 6, "name": "부분 염색", "price": 50000, "duration": 60}]'::jsonb,
 60, 50000, 'COMPLETED', 'RES-20260102-0001', '뿌리만 해주세요', ''),
...
(8, 2, 6, 3, '2026-01-03', '10:00', '10:50',
 ...
 50, 80000, 'COMPLETED', 'RES-20260103-0001', '', '척추 중립 자세 교정'),
...
-- ============================================
-- 10. 고객 시술 이력 (Customer Histories)
-- ============================================
...
RAISE NOTICE '매장설정: 2개 (각 매장별 상세 설정)';
RAISE NOTICE '직원: 4명';
